...
TILE_HEIGHT = 10.0
RIVER_WIDTH = 200.0 
TRAP_PULSE_SPEED = 8.0 # How fast the trap tile pulsates
TRAP_FUSE_TIME = 3.0 # Seconds before a trap tile triggers game over
...
def reset_game():
    ...
    # --- Starting tile (wider and distinct color) ---
    START_TILE_WIDTH = TILE_SIZE * 2  # Twice as wide
    tiles.append({
        'pos': [0, 0, 0],
        'size': START_TILE_WIDTH,  # wider than other tiles
        'type': 'safe',
        'color': [0.7, 0.7, 0.7],  # slightly different shade
        'origin_x': 0
    })

    # Generate initial subsequent tiles
    generate_new_tile()
    generate_new_tile()
    generate_new_tile()
...
def generate_new_tile():
    ...
    tile_type = 'safe'
    # NEW: All tiles start with the safe grey color
    color = [0.5, 0.5, 0.5]
    ...
    if score > 15:
        ...
        if rand_choice < 0.3:
            ...
            tile_type = 'moving'
            color = [0.0, 0.8, 1.0] # Moving tiles are still colored
    elif score > 5:
...
def draw_player():
    """Draws the player with different poses depending on jump state."""
    glPushMatrix()
    ...
    if jumping:
        ...
        for offset in [-7, 7]:
            glPushMatrix()
            glTranslatef(offset, leg_height + 5 + torso_height - 5, 0)
            glRotatef(90, 1, 0, 0)
            gluCylinder(gluNewQuadric(), 2.0, 2.0, arm_height, 10, 10)
            glPopMatrix()
...
def draw_tiles_with_outlines():
    """Draws all tiles without outlines (just solid colored cubes)."""
    for tile in tiles:
        glPushMatrix()
...
def draw_environment():
    """Draws a simple sea with no riverbanks or waves."""
    glColor3f(0.0, 0.3, 0.6)  # deep sea blue
    glBegin(GL_QUADS)
    glVertex3f(-RIVER_WIDTH*5, -TILE_HEIGHT/2, 500)
    glVertex3f(RIVER_WIDTH*5, -TILE_HEIGHT/2, 500)
    glVertex3f(RIVER_WIDTH*5, -TILE_HEIGHT/2, -5000)
    glVertex3f(-RIVER_WIDTH*5, -TILE_HEIGHT/2, -5000)
    glEnd()
...
def update_game_state():
    ...
    global game_state, player_pos, score
    ...
    for tile in tiles:
        if tile.get('is_active'):
            ...
            # Pulsate color using a sine wave
    ...
    if game_state == 'JUMPING':
        ...
        for tile in tiles:
            ...
            if dist_x < tile['size'] / 2 and dist_z < tile['size'] / 2:
                ...
                # NEW: Check if the landed tile is a trap
        ...
        if landed_safely:
            # If we landed on a trap, we don't score or generate a new tile yet
            if not active_trap:
                score += 1
                generate_new_tile()
                game_state = 'AIMING'
            else:
                # We are on a trap, so we stay in 'LANDED' state but can aim again
                game_state = 'AIMING'
...
def showScreen():
    ...
    draw_environment()
    draw_tiles_with_outlines() # UPDATED to new drawing function
    draw_player()
    draw_aiming_arrow()
